...
class BayesianDropoutModel(Model):
    ...
    def predict_stochastic(self, X, batch_size=1, verbose=False):
        ...
        if self._predict_stochastic is None:
            # self.validate()
            self._predict_stochastic = K.function([self.inputs[self.num_input]], [self.outputs[self.num_output]],
                                                  givens={K.learning_phase(): np.uint8(1)})
        if not isinstance(X, list):
            X = [X]
        return self._predict_loop(self._predict_stochastic, X, batch_size=batch_size, verbose=verbose)
